...
import { getEstadoDocs, getEstadoDocById } from 'store/slices/estadoDoc';
...
export default function DropEstado({
    Id,
    SetFieldValue,
    Value,
    SetValue,
    ...
    if (estadoDocs.rows) {
        estadoDocs.rows.forEach((result, index) => {
            /* eslint no-underscore-dangle: 0 */
            data[index] = { title: result.descripcion, value: result._id };
        });
    }

    console.log('estadoDocs', estadoDoc);
                ...
                <Autocomplete
                    ...
                    onChange={(event, value) => {
                        if (value.value) {
                            ...
                            let datosestadoDoc;
                            if (estadoDocs.rows) {
                                datosestadoDoc = estadoDocs.rows.find((result) => result._id === Values.value);
                            }
                            // alert(JSON.stringify(datosestadoDoc));
                            SetDatosestadoDoc(datosestadoDoc);
                        } else {
                            SetFieldValue(Id, '');
                        }
                        // dispatch(getestadoDocById(value.value));
                    }}
                    renderInput={(params) => (
                        <TextField
                            {...params}
