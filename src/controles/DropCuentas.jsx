...
import Cuenta, { getCuentaById, getCuentas } from 'store/slices/cuenta';
...
export default function DropCuentas({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { cuentas, cuenta, error } = useSelector((state) => state.cuenta);
    ...
    if (cuentas.rows) {
        cuentas.rows.forEach((result, index) => {
