...
export default function DropUnidadMedida({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange, Row }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { unidadMedida, unidadMedidas, error } = useSelector((state) => state.unidadMedida);
    // const { paginas } = useSelector((state) => state.pagina);
    React.useEffect(() => {
        dispatch(getUnidadMedida());
    }, [dispatch]);

    if (unidadMedidas.rows) {
        unidadMedidas.rows.forEach((result, index) => {
            ...
            <Grid item xs={12}>
                <Autocomplete
                    ...
                    onChange={(event, value) => {
                        ...
                        Onchange(value.value, Row, SetFieldValue, Id);
                    }}
                    renderInput={(params) => (
                        <TextField
                            {...params}
