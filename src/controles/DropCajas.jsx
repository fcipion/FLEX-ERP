...
import caja, { getCajas, getCajaById } from 'store/slices/caja';
...
export default function DropCajas({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { cajas, cajae, error } = useSelector((state) => state.caja);
    ...
    if (cajas.rows) {
        cajas.rows.forEach((result, index) => {
