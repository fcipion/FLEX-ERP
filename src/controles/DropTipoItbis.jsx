...
export default function DropTipoItbis({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange, Row }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { tipoitbis, tipoitbiss, error } = useSelector((state) => state.tipoitbis);
    ...
    if (tipoitbiss.rows) {
        tipoitbiss.rows.forEach((result, index) => {
