...
import condicionesPago, { getCondicionesPagos, getCondicionesPagoById } from 'store/slices/condicionesPago';
// import condicionesPagos from 'componetes/Gestion/condicionesPagos';
...
export default function DropCodicionPago({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { condicionesPagos, condicionesPagoe, error } = useSelector((state) => state.condicionesPago);
    // const { paginas } = useSelector((state) => state.pagina);
    React.useEffect(() => {
        dispatch(getCondicionesPagos());
    }, [dispatch]);

    if (condicionesPagos.rows) {
        condicionesPagos.rows.forEach((result, index) => {
