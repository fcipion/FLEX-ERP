...
export default function DropProductos({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange, Row }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { producto, productos, error } = useSelector((state) => state.producto);
    ...
    if (productos.rows) {
        productos.rows.forEach((result, index) => {
                ...
                <Autocomplete
                    ...
                    onChange={(event, value) => {
                        ...
                        if (value.value) {
                            ...
                            if (productos.rows) {
                                /* eslint no-underscore-dangle: 0 */
                                datosProducto = productos.rows.find((result) => result._id === value.value);
                            }
                        } else {
                            SetFieldValue(Id, '');
                        }
                        Onchange(value.value, Row, SetFieldValue, Id, datosProducto);
                    }}
                    renderInput={(params) => (
                        <TextField
                            {...params}
                            ...
                            InputProps={{
                                ...
                                endAdornment: (
                                    <>
                                        ...
                                        <IconButton onClick={handleClickOpen} aria-label="Agregar" size="small">
                                            <AddCircleTwoToneIcon size="small" titleAccess="Agregar" />
                                        </IconButton>
                                    </>
                                )
                            }}
