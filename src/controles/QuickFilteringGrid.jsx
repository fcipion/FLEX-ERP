...
import { capitalizeString, uppercaseFirstLetter } from "utils/string.utils";

const excludedColumns = ["descripcion", "id", "estatus", "celular"];
...
export default function QuickFilteringGrid({
  ...
  const actions = {
    ...
    getActions: (params) => [
      <GridActionsCellItem
        icon={
          <RemoveRedEyeTwoToneIcon sx={{ fontSize: "1.3rem" }} color="info" />
        }
        label="Ver"
        onClick={() => {
  ...
  const columns = [
    ...data.columns.filter((x) => !excludedColumns.includes(x.field)),
  ];
  // columns[1].flex = '1';
  ...
  // const columnsData = [id, nombre, ...columns.map(x => ({
  //     field: x.field,
  //     headerName: capitalizeString(x.field),
  //     headerClassName: 'headerStyle'
  // })), estatus, actions];
