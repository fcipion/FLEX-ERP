...
export default function DropMonedas({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange, Row }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { moneda, monedas, error } = useSelector((state) => state.moneda);
    ...
    if (monedas.rows) {
        monedas.rows.forEach((result, index) => {
