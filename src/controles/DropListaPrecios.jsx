...
export default function DropListaPrecios({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange, Row, variant }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { listaPrecio, listaPrecios, error } = useSelector((state) => state.listaPrecio);
    ...
    if (listaPrecios.rows) {
        listaPrecios.rows.forEach((result, index) => {
            ...
            <Grid item xs={12}>
                <Autocomplete
                    ...
                    onChange={(event, value) => {
                        ...
                        //                 /* eslint no-underscore-dangle: 0 */
                        //                 (result) => result.producto._id === Row.producto
                        //             );
                        //         }
                        //     }
                        ...
                        //     SetFieldValue(Id, '');
                        // }
                        Onchange(value.value, Row, SetFieldValue, Id, resultadoDatosDeterminacion);
                    }}
                    renderInput={(params) => (
                        <TextField
                            {...params}
