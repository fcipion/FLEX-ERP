...
import tipoIngreso, { getTipoIngresos, getTipoIngresoById } from 'store/slices/tipoIngreso';
import TipoIngresos from 'componetes/administracion/TipoIngresos';
// import tipoIngresoes from 'componetes/administracion/tipoIngresoes';
...
export default function DroptipoIngreso({ Id, SetFieldValue, Value, SetValue, Label, Errors, Touched, OnBlur, Onchange }) {
    const [openDialog, setOpenDialog] = React.useState(false);
    const [scroll, setScroll] = React.useState('paper');
    const [open, setOpen] = React.useState(false);
    const [options, setOptions] = React.useState([]);
    const loading = open && options.length === 0;
    const { tipoIngreso, tipoIngresos, error } = useSelector((state) => state.tipoIngreso);
    // const { paginas } = useSelector((state) => state.pagina);
    React.useEffect(() => {
        dispatch(getTipoIngresos());
    }, [dispatch]);

    if (tipoIngresos.rows) {
        tipoIngresos.rows.forEach((result, index) => {
