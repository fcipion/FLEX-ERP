...
import { getRols, getRolById } from 'store/slices/rol';
...
const Roles = () => {
    ...
    const { rols, rol, error } = useSelector((state) => state.rol);
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de rol';
            break;
        case 'edit':
            formTitulo = 'Editar rol';
            break;

        case 'view':
            formTitulo = 'Visualizar rol';
            break;
        default:
            formTitulo = 'Registro de rol';
            break;
        ...
        <>
            ...
            {rols.length !== 0 ? (
                <MainCard title="Listado de rol">
                    <QuickFilteringGrid data={rols} clickAdd={handlerAdd} clickEdit={clickEdit} clickView={clickView} />
                </MainCard>
            ) : (
                <LineProgress />
            )}
            ...
            >
                <Formik
                    ...
                    initialValues={{
                        ...
                        estatus: rol.data ? rol.data.estatus : true
                    }}
                    ...
                    onSubmit={(value, { setSubmitting, resetForm }) => {
                        setTimeout(async () => {
                            ...
                            try {
                                switch (modoAccion) {
                                    ...
                                    case 'Editar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizada la rol: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    ...
                                    case 'Copiar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Copiada la rol: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'delete':
                                ...
                                setSubmitting(false);
                                setMessageInfo({ type: 'error', title: `Error creando rol: ${JSON.stringify(error)}` });
                            }
                    ...
                    {({ values, errors, touched, isSubmitting, setFieldValue, handleChange, handleSubmit, handleBlur }) => {
                        ...
                        const handlerNavegateData = (value) => {
                            if (modo === 'edit' || modo === 'view') {
                                ...
                                if (idNexBack !== 0) {
                                    navegate(`/rol/edit/${idNexBack}/${generateId()}`);
                                }
                            }
                            //  else {
