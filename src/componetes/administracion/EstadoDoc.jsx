...
import { getEstadoDocById, getEstadoDocs } from 'store/slices/estadoDoc';
...
const EstadoDoc = () => {
    ...
    const { estadoDocs, estadoDoc, error } = useSelector((state) => state.estadoDoc);
    // const { paginas } = useSelector((state) => state.pagina);
    React.useEffect(() => {
        dispatch(getEstadoDocs());
    }, [dispatch]);

    React.useEffect(() => {
        dispatch(getEstadoDocById(id));
    }, [id, accion, modo]);
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de estado documentos';
            break;
        case 'edit':
            formTitulo = 'Editar estado documentos';
            break;

        case 'view':
            formTitulo = 'Visualizar estado documentos';
            break;
        default:
            formTitulo = 'Registro de estado documentos';
            break;
    ...
    return modo === 'Index' ? (
        <>
            {console.log('rols', estadoDocs)}
            {estadoDocs.length !== 0 ? (
                <MainCard title="Listado de estado documentos">
                    <QuickFilteringGrid data={estadoDocs} clickAdd={handlerAdd} clickEdit={clickEdit} clickView={clickView} />
                </MainCard>
            ) : (
                <LineProgress />
            )}
            ...
            >
                <Formik
                    ...
                    initialValues={{
                        ...
                        descripcion: estadoDoc.data ? estadoDoc.data.descripcion : '',
                        code: estadoDoc.data ? estadoDoc.data.code : '',
                        estatus: estadoDoc.data ? estadoDoc.data.estatus : true
                    }}
                    ...
                    onSubmit={(value, { setSubmitting, resetForm }) => {
                        setTimeout(async () => {
                            ...
                            try {
                                switch (modoAccion) {
                                    case 'Crear':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Creada el estado: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'Crear nuevo':
                                    ...
                                    case 'Editar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizado el estado: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    ...
                                    case 'Copiar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Copiado el estado: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'delete':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'warning',
                                                title: `estado eliminada: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    default:
                                ...
                                setMessageInfo({
                                    type: 'error',
                                    title: `Error creando estado ${JSON.stringify(error)}`
                                });
                            }
                    ...
                    {({ values, errors, touched, isSubmitting, setFieldValue, handleChange, handleSubmit, handleBlur }) => {
                        ...
                        const handlerNavegateData = (value) => {
                            if (modo === 'edit' || modo === 'view') {
                                ...
                                if (idNexBack !== 0) {
                                    navegate(`/estadoDoc/edit/${idNexBack}/${generateId()}`);
                                }
                            }
                            //  else {
