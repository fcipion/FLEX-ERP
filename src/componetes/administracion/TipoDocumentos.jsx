...
import { getTipoDocumentos, getTipoDocumentosById } from 'store/slices/tipoDocumento';
...
const TipoDocumentos = () => {
    ...
    const { tipoDocumentos, tipoDocumento, error } = useSelector((state) => state.tipoDocumento);
    ...
    React.useEffect(() => {
        dispatch(getTipoDocumentosById(id));
    }, [id, accion, modo]);
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de Tipo de documentos';
            break;
        case 'edit':
            formTitulo = 'Editar Tipo de documentos';
            break;

        case 'view':
            formTitulo = 'Visualizar Tipo de documentos';
            break;
        default:
            formTitulo = 'Registro de Tipo de documentos';
            break;
    ...
    return modo === 'Index' ? (
        <>
            {console.log('rols', tipoDocumentos)}
            {tipoDocumentos.length !== 0 ? (
                <MainCard title="Listado de Tipo de documentos">
                    <QuickFilteringGrid data={tipoDocumentos} clickAdd={handlerAdd} clickEdit={clickEdit} clickView={clickView} />
                </MainCard>
            ) : (
                <LineProgress />
            )}
            ...
            >
                <Formik
                    ...
                    initialValues={{
                        ...
                        descripcion: tipoDocumento.data ? tipoDocumento.data.descripcion : '',
                        estatus: tipoDocumento.data ? tipoDocumento.data.estatus : true
                    }}
                    ...
                    onSubmit={(value, { setSubmitting, resetForm }) => {
                        setTimeout(async () => {
                            ...
                            try {
                                switch (modoAccion) {
                                    case 'Crear':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Creada el tipo de documento: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'Crear nuevo':
                                    ...
                                    case 'Editar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizado el tipo de documento: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    ...
                                    case 'Copiar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Copiado el tipo de documento: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'delete':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'warning',
                                                title: `Tipo de documento eliminada: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    default:
                                ...
                                setMessageInfo({
                                    type: 'error',
                                    title: `Error creando Tipo de documento ${JSON.stringify(error)}`
                                });
                            }
                    ...
                    {({ values, errors, touched, isSubmitting, setFieldValue, handleChange, handleSubmit, handleBlur }) => {
                        ...
                        const handlerNavegateData = (value) => {
                            if (modo === 'edit' || modo === 'view') {
                                ...
                                if (idNexBack !== 0) {
                                    navegate(`/tipoDocumento/edit/${idNexBack}/${generateId()}`);
                                }
                            }
                            //  else {
