...
import { getClaseReprComerciales, getClaseReprComercialesById } from 'store/slices/claseReprComerciales';
...
const TipoCP = () => {
    ...
    const { claseReprComerciales, claseReprComerciale, error } = useSelector((state) => state.claseReprComerciale);
    // const { paginas } = useSelector((state) => state.pagina);
    React.useEffect(() => {
        dispatch(getClaseReprComerciales());
    }, [dispatch]);

    React.useEffect(() => {
        dispatch(getClaseReprComercialesById(id));
    }, [id, accion, modo]);
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de Clase de representantes comerciales';
            break;
        case 'edit':
            formTitulo = 'Editar Clase de representantes comerciales';
            break;

        case 'view':
            formTitulo = 'Visualizar Clase de representantes comerciales';
            break;
        default:
            formTitulo = 'Registro de Clase de representantes comerciales';
            break;
    ...
    return modo === 'Index' ? (
        <>
            {console.log('rols', claseReprComerciales)}
            {claseReprComerciales.length !== 0 ? (
                <MainCard title="Listado de Clase de representantes comerciales">
                    <QuickFilteringGrid data={claseReprComerciales} clickAdd={handlerAdd} clickEdit={clickEdit} clickView={clickView} />
                </MainCard>
            ) : (
                <LineProgress />
            )}
            ...
            >
                <Formik
                    ...
                    initialValues={{
                        ...
                        descripcion: claseReprComerciale.data ? claseReprComerciale.data.descripcion : '',
                        estatus: claseReprComerciale.data ? claseReprComerciale.data.estatus : true
                    }}
                    ...
                    onSubmit={(value, { setSubmitting, resetForm }) => {
                        setTimeout(async () => {
                            ...
                            try {
                                switch (modoAccion) {
                                    case 'Crear':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Creada la clase de representante comercial: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'Crear nuevo':
                                    ...
                                    case 'Editar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizado la clase de representante comercial: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    ...
                                    case 'Copiar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Copiado la clase de representante comercial: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'delete':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'warning',
                                                title: `Clase de representante comercial eliminada: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    default:
                                ...
                                setMessageInfo({
                                    type: 'error',
                                    title: `Error creando Clase de representante comercial ${JSON.stringify(error)}`
                                });
                            }
                        ...
                        const handlerNavegateData = (value) => {
                            if (modo === 'edit' || modo === 'view') {
                                const idNexBack = getNextValue(
                                    // Arreglo de rols
                                    claseReprComerciales.rows,
                                    // Objeto rols
                                    claseReprComerciale.data,
                                    value
                                );
                                if (idNexBack === 0) setAlert({ type: 'warning', open: true, message: 'Registro no econtrado' });
                                if (idNexBack !== 0) {
                                    navegate(`/claseReprComerciale/edit/${idNexBack}/${generateId()}`);
                                }
                            }
                            //  else {
