      ...
      >
        <Formik
          ...
          initialValues={{
            ...
            estatus: doctor.data ? doctor.data.estatus : true,
          }}
          ...
          onSubmit={(value, { setSubmitting, resetForm }) => {
            setTimeout(async () => {
              ...
              try {
                switch (modoAccion) {
                  case "Crear":
                    ...
                    if (!result.error) {
                      setMessageInfo({
                        type: "success",
                        title: `Creada el doctor: ${result.data.data.nombre}`,
                      });
                    }
                    break;
                  case "Crear nuevo":
                  ...
                  case "Crear editar":
                    ...
                    /* eslint no-underscore-dangle: 0 */
                    navegate(
                      `/doctor/edit/${result.data.data._id}/${generateId()}`
                    );
                  ...
                  case "Editar":
                    ...
                    if (!result.error) {
                      setMessageInfo({
                        type: "success",
                        title: `Actualizada el doctor: ${result.data.data.nombre}`,
                      });
                    }
                    break;
                  ...
                  case "Copiar":
                    ...
                    if (!result.error) {
                      setMessageInfo({
                        type: "success",
                        title: `Copiada el doctor: ${result.data.data.nombre}`,
                      });
                    }
                    break;
                  case "delete":
                    result = await axios.delete(
                      `${url}/eliminar_doctor/${id}`,
                      value
                    );
                    ...
                    if (!result.error) {
                      setMessageInfo({
                        type: "warning",
                        title: `doctor eliminado: ${result.data.data.nombre}`,
                      });
                    }
                    break;
                  default:
                ...
                setMessageInfo({
                  type: "error",
                  title: `Error creando doctor: ${JSON.stringify(error)}`,
                });
              }
