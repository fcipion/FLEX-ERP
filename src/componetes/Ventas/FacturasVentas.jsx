...
const FacturasVentas = () => {
    ...
    const { producto, productos } = useSelector((state) => state.producto);
    ...
    // React.useEffect(() => {
    //     setParametros({ pmodo: modo, pid: id, paccion: accion });
    // }, accion);
    React.useEffect(() => {
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de Factura de Cliente';
            break;
        case 'edit':
            formTitulo = 'Editar factura de Cliente';
            break;

        case 'view':
            formTitulo = 'Visualizar factura de Cliente';
            break;
        default:
            formTitulo = 'Registro de factura de cliente';
            break;
    ...
    const camposRequeridos = (values) => {
        ...
        if (values.cliente === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Cliente requerido';
                return previewValue;
            });
        ...
        // Validar sucursal;
        if (values.sucursal === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Sucursal requerida';
                return previewValue;
            });
        ...
        // // Validar vendedor;
        // if (values.vendedor === null) {
        //     setCamposRequerdio((previewValue) => {
        //         previewValue.descripcion = 'Vendedor requerido';
        //         return previewValue;
        //     });
        ...
        // if (values.caja === null) {
        //     setCamposRequerdio((previewValue) => {
        //         previewValue.descripcion = 'Caja requerido';
        //         return previewValue;
        //     });
        ...
        // Validar doctor;
        if (values.doctor === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Doctor requerido';
                return previewValue;
            });
        ...
        // Validar montoAplicado;
        if (values.montoAplicado === 0) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Por favor aplicar el pago...';
                return previewValue;
            });
        ...
        // Validar totalPendiente;
        console.log('Documento', JSON.stringify(values));
        if (values.totalPendiente !== 0) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'por favor pagar  el documento en su totalidad...';
                return previewValue;
            });
        ...
        if (values.subtotal === 0) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Por favor agregar la lineas del documento';
                return previewValue;
            });
        ...
        if (values.fecha_vencimiento === '') {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Por colocar la fecha de vencimiento';
                return previewValue;
            });
        ...
        // Validar fecha_contabilizacion;
        if (values.fecha_contabilizacion === '') {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Por colocar la fecha de contabilización';
                return previewValue;
            });
        ...
        // Validar fecha_contabilizacion;
        if (values.moneda === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'La moneda es requerida...';
                return previewValue;
            });
        ...
        // Validar fecha_contabilizacion;
        if (values.tipo_cambio === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'El tipo de cambio es requerido...';
                return previewValue;
            });
        ...
        // Validar condicion_pago;
        if (values.condicion_pago === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'La condición de pago es requerida...';
                return previewValue;
            });
        ...
        // Si selecciona un tipo de comprobante debes seleccionar los datos correspondientes;
        if (values.tipo_comprobante !== null) {
            // Validar condicion_pago;
            if (values.rnc === null) {
                setCamposRequerdio((previewValue) => {
                    previewValue.descripcion = 'El rnc es requerido para la factura con comprobantes...';
                    return previewValue;
                });
        ...
        if (values.tipo_ingreso === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'El tipo de ingreso es requerido..';
                return previewValue;
            });
        ...
        // Validar tipo_ingreso;
        if (values.medio_pago === null) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'El medio de pago es requerido..';
                return previewValue;
            });
        ...
        if (!values.detalles) {
            setCamposRequerdio((previewValue) => {
                previewValue.descripcion = 'Por favor agregar un detalle a este documento..';
                return previewValue;
            });
        }
        ...
        <>
            ...
            >
                <Formik
                    enableReinitialize
                    initialValues={{
                        compania: venta.data ? venta.data.compania : null, // facturas.compania,
                        sucursal: venta.data ? venta.data.sucursal : null, // facturas.sucursal,
                        cliente: venta.data ? venta.data.cliente : null, // facturas.cliente,
                        vendedor: venta.data ? venta.data.vendedor : JSON.parse(localStorage.getItem('userData')).sub,
                        caja: venta.data ? venta.data.caja : null, // facturas.caja,
                        doctor: venta.data ? venta.data.doctor : null, // facturas.doctor,
                        totalDescuentos: venta.data ? venta.data.totalDescuentos : 0, // facturas.totalDescuentos,
                        montoAplicado: venta.data ? venta.data.montoAplicado : 0, // facturas.montoAplicado,
                        subtotal: venta.data ? venta.data.subtotal : 0, // venta.data ? venta.data.subtotal : 0, // facturas.subtotal,
                        itbis: venta.data ? venta.data.itbis : 0, // facturas.itbis,
                        total: venta.data ? venta.data.total : 0, // facturas.total,
                        descuentosCB: venta.data ? venta.data.descuentosCB : 0, //  facturas.descuentosCB,
                        fecha_vencimiento: venta.data ? venta.data.fecha_vencimiento : Date.now(), // facturas.fecha_vencimiento,
                        fecha_contabilizacion: venta.data ? venta.data.fecha_contabilizacion : Date.now(), // facturas.fecha_contabilizacion,
                        comentarios: venta.data ? venta.data.comentarios : '', // facturas.comentarios,
                        moneda: venta.data ? venta.data.moneda : null, // facturas.moneda,
                        tipo_cambio: venta.data ? venta.data.tipo_cambio : null, // facturas.tipo_cambio,
                        condicion_pago: venta.data ? venta.data.condicion_pago : null, // facturas.condicion_pago,
                        tipo_comprobante: venta.data ? venta.data.tipo_comprobante : null, // facturas.tipo_comprobante,
                        ncf: venta.data ? venta.data.ncf : '', // facturas.ncf,
                        fecha_vencimiento_ncf: venta.data ? venta.data.fecha_vencimiento_ncf : '', // facturas.fecha_vencimiento,
                        rnc: venta.data ? venta.data.rnc : '', // facturas.rnc,
                        tipo_ingreso: venta.data ? venta.data.tipo_ingreso : null, // facturas.tipo_ingreso,
                        medio_pago: venta.data ? venta.data.medio_pago : null, // facturas.medio_pago,
                        detalles: venta.data ? venta.data.detalles : [], // facturas.detalles,
                        proceso_medio_pago: venta.data ? venta.data.proceso_medio_pago : inicialState.proceso_medio_pago, // facturas.proceso_medio_pago,
                        totalPendiente: venta.data ? venta.data.totalPendiente : 0,
                        reporte: false,
                        estatus: true,
                        cancelada: false
                    }}
                                        ...
                                        <MainCard title={formTitulo}>
                                            ...
                                            <CrudControl
                                                ...
                                                AplicarPago={() => {
                                                    if (facturas.subtotal === 0) {
                                                        alert('No se puede aplicar un pago a una factura con valor 0');
                                                    } else {
                                                        setValuePago(true);
                                                    }
                                                }}
                                            ...
                                            >
                                                ...
                                                <ControlDetalleDocumento
                                                    ...
                                                    determinacionPrecios={determinacionPrecios}
                                                    productos={productos}
                                                    itbiss={itbiss}
                                                    ventas={ventas}
                                                    venta={venta}
