...
import { getTComprobantes, getTComprobantesById } from 'store/slices/tiposComprobante';
...
const TipoComprobantes = () => {
    ...
    const { tComprobantes, tComprobante, error } = useSelector((state) => state.tComprobante);
    ...
    React.useEffect(() => {
        dispatch(getTComprobantesById(id));
    }, [id, accion, modo]);
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de tipo de comprobantes';
            break;
        case 'edit':
            formTitulo = 'Editar tipo de comprobantes';
            break;

        case 'view':
            formTitulo = 'Visualizar tipo de comprobantes';
            break;
        default:
            formTitulo = 'Registro de tipo de comprobantes';
            break;
    ...
    return modo === 'Index' ? (
        <>
            {tComprobantes.length !== 0 ? (
                <MainCard title="Listado de tipo de comprobantes">
                    <QuickFilteringGrid data={tComprobantes} clickAdd={handlerAdd} clickEdit={clickEdit} clickView={clickView} />
                </MainCard>
            ) : (
                <LineProgress />
            )}
            ...
            >
                <Formik
                    ...
                    initialValues={{
                        ...
                        tipo: tComprobante.data ? tComprobante.data.tipo : '',
                        descripcion: tComprobante.data ? tComprobante.data.descripcion : '',
                        estatus: tComprobante.data ? tComprobante.data.estatus : true
                    }}
                    ...
                    onSubmit={(value, { setSubmitting, resetForm }) => {
                        setTimeout(async () => {
                            ...
                            try {
                                switch (modoAccion) {
                                    case 'Crear':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Creado el tipo de comprobante: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'Crear nuevo':
                                    ...
                                    case 'Editar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizada el tipo de comprobante: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    ...
                                    case 'Copiar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Copiada el tipo de comprobante: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'delete':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'warning',
                                                title: `tipo de comprobante eliminado: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    default:
                                ...
                                setSubmitting(false);
                                setMessageInfo({ type: 'error', title: `Error creando tipo de comprobante: ${JSON.stringify(error)}` });
                            }
                    ...
                    {({ values, errors, touched, isSubmitting, setFieldValue, handleChange, handleSubmit, handleBlur }) => {
                        ...
                        const handlerNavegateData = (value) => {
                            if (modo === 'edit' || modo === 'view') {
                                ...
                                if (idNexBack !== 0) {
                                    navegate(`/tipoComprobante/edit/${idNexBack}/${generateId()}`);
                                }
                            }
                            //  else {
