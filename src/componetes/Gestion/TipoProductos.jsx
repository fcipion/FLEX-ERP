...
import { getTipoProductos, getTipoProductosById } from 'store/slices/tipoProducto';
...
const TipoProductos = () => {
    ...
    const { tipoProductos, tipoProducto, error } = useSelector((state) => state.tipoProducto);
    ...
    React.useEffect(() => {
        dispatch(getTipoProductosById(id));
    }, [id, accion, modo]);
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de tipo de productos';
            break;
        case 'edit':
            formTitulo = 'Editar tipo de productos';
            break;

        case 'view':
            formTitulo = 'Visualizar tipo de productos';
            break;
        default:
            formTitulo = 'Registro de tipo de productos';
            break;
    ...
    return modo === 'Index' ? (
        <>
            {console.log('rols', tipoProductos)}
            {tipoProductos.length !== 0 ? (
                <MainCard title="Listado de tipo de productos">
                    <QuickFilteringGrid data={tipoProductos} clickAdd={handlerAdd} clickEdit={clickEdit} clickView={clickView} />
                </MainCard>
            ) : (
                <LineProgress />
            )}
            ...
            >
                <Formik
                    ...
                    initialValues={{
                        ...
                        descripcion: tipoProducto.data ? tipoProducto.data.descripcion : '',
                        estatus: tipoProducto.data ? tipoProducto.data.estatus : true
                    }}
                    ...
                    onSubmit={(value, { setSubmitting, resetForm }) => {
                        setTimeout(async () => {
                            ...
                            try {
                                switch (modoAccion) {
                                    case 'Crear':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Creada el tipo de producto: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'Crear nuevo':
                                    ...
                                    case 'Editar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizado el tipo de producto: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    ...
                                    case 'Copiar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Copiado el tipo de producto: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'delete':
                                ...
                                setSubmitting(false);
                                setMessageInfo({ type: 'error', title: `Error creando tipo de producto: ${JSON.stringify(error)}` });
                            }
                    ...
                    {({ values, errors, touched, isSubmitting, setFieldValue, handleChange, handleSubmit, handleBlur }) => {
                        ...
                        const handlerNavegateData = (value) => {
                            if (modo === 'edit' || modo === 'view') {
                                ...
                                if (idNexBack !== 0) {
                                    navegate(`/tipoProducto/edit/${idNexBack}/${generateId()}`);
                                }
                            }
                            //  else {
