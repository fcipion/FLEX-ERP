...
import { getBancos, getBancoById } from 'store/slices/banco';
...
const Bancos = () => {
    ...
    const { bancos, banco, error } = useSelector((state) => state.banco);
    ...
    switch (modo) {
        case 'create':
            formTitulo = 'Registro de banco';
            break;
        case 'edit':
            formTitulo = 'Editar banco';
            break;

        case 'view':
            formTitulo = 'Visualizar banco';
            break;
        default:
            formTitulo = 'Registro de banco';
            break;
    ...
    return modo === 'Index' ? (
        <>
            ...
            {bancos.length !== 0 ? (
                <MainCard title="Listado de banco">
                    <QuickFilteringGrid data={bancos} clickAdd={handlerAdd} clickEdit={clickEdit} clickView={clickView} />
                </MainCard>
            ) : (
                <LineProgress />
            )}
            ...
            >
                <Formik
                    ...
                    initialValues={{
                        ...
                        imagen: banco.imagen ? banco.data.imagen : '',
                        sucursal: banco.data ? banco.data.sucursal : '',
                        descripcion: banco.data ? banco.data.descripcion : '',
                        estatus: banco.data ? banco.data.estatus : true
                    }}
                    ...
                    onSubmit={(value, { setSubmitting, resetForm }) => {
                        setTimeout(async () => {
                            ...
                            try {
                                switch (modoAccion) {
                                    case 'Crear':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Creado el banco: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'Crear nuevo':
                                    ...
                                    case 'Editar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizada el banco: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    ...
                                    case 'Editar nuevo':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Actualizado el banco: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'Copiar':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'success',
                                                title: `Copiado el banco: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    case 'delete':
                                        ...
                                        if (!result.error) {
                                            setMessageInfo({
                                                type: 'warning',
                                                title: `Banco eliminada: ${result.data.data.descripcion}`
                                            });
                                        }
                                        break;
                                    default:
                                ...
                                setSubmitting(false);
                                setMessageInfo({ type: 'error', title: `Error creando el banco: ${JSON.stringify(error)}` });
                            }
                    ...
                    {({ values, errors, touched, isSubmitting, setFieldValue, handleChange, handleSubmit, handleBlur }) => {
                        ...
                        const handlerNavegateData = (value) => {
                            if (modo === 'edit' || modo === 'view') {
                                ...
                                if (idNexBack !== 0) {
                                    navegate(`/banco/edit/${idNexBack}/${generateId()}`);
                                }
                            }
                            //  else {
